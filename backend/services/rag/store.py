...
_PERSIST_DIR = CFG.paths.chroma_dir
...
def has_timestamp_index(video_id: str) -> bool:
    """Return True when the collection matches the current subtitle RAG index."""
    store = get_vector_store(video_id)
    if store._collection.count() <= 0:
        return False
    raw = store.get(limit=1, include=["metadatas"])
    metas = raw.get("metadatas") or []
    if not metas:
        return False
    meta = metas[0] or {}
    return (
        meta.get("rag_version") == "subtitle-v1"
        and meta.get("start") is not None
        and meta.get("end") is not None
        and meta.get("embedding_provider") == CFG.embedding.provider
        and meta.get("embedding_model") == CFG.embedding.model
    )


def reset_vector_store(video_id: str) -> None:
    store = get_vector_store(video_id)
    if store._collection.count() <= 0:
        return
